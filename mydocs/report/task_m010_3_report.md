...
- 마일스톤: M010
- 단계 수: 5
- 작업 목적: local Codex session JSONL parser를 구현해 `UsageSnapshot v2` production output에 실제 usage/activity aggregate를 연결한다.
...
| `src/parser/session-jsonl.js` | session JSONL discovery, streaming read, token_count normalizer, nested usage alias 지원 | 모든 local parser aggregate 입력 |
| `src/parser/token-aggregate.js` | token total, breakdown, daily bucket, peak daily aggregate 구현 | `usage` field |
| `src/parser/model-aggregate.js` | model별 token/usage ranking aggregate 구현 | `models` field, source 없으면 unavailable |
| `src/parser/activity-aggregate.js` | totalThreads, duration, streak, reasoning effort aggregate 구현 | `activity` field |
| `src/analyze.js` | parser aggregate를 production `UsageSnapshot v2`에 병합 | SDK production output |
| `src/cli.js` | `--codex-home <path>` option 추가, sample fixture 분리 유지 | CLI test/smoke와 custom source 주입 |
| `src/index.d.ts` | `AnalyzeUsageOptions`에 `codexHome`, `now` option 반영 | TypeScript 소비자 |
...
| `README.md` | production parser, unavailable baseline, fixture sample 경계 설명 | 사용자/기여자 문서 |
| `mydocs/working/task_m010_3_stage1.md` ~ `task_m010_3_stage5.md` | 단계별 검증과 판단 기록 | 하이퍼-워터폴 작업 문서 |
...
| `mydocs/working/task_m010_3_stage{N}.md` | `mydocs/working/` | `mydocs/working/task_m010_3_stage1.md` ~ `task_m010_3_stage5.md` | OK | 단계 보고서 위치와 파일명 규칙 일치 |
...
| diagnostic에 raw local path, raw line, session id, prompt/response 원문 미노출 | OK — parser tests와 Stage 5 privacy scanner 통과 |
| 실제 environment smoke 수행 | OK — raw JSON 미보관 원칙으로 schema/privacy/status만 확인 |
...
- 실제 local source에서 token event에 model field가 없어 `models`는 unavailable로 남는다.
...
- #4 또는 별도 후속: 실제 model source alias/event 조사 및 `models` aggregate 보강
...
- 별도 후속: large session directory scan 성능 최적화와 incremental/cache 전략 검토

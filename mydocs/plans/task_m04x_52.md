...
현재 명시적 opt-in 실험 기능인 `profile`에 `--include-pet` 옵션을 추가해, 사용자가 별도로 동의한 경우에만 선택된 custom Codex pet의 제한된 메타데이터와 이미지 payload를 제공한다. 기본값은 Codex Desktop에서 이미 선택된 custom pet이며, 사용자가 CLI selector 또는 module catalog key로 다른 설치 custom pet을 명시적으로 고를 수도 있다. 플래그 없는 `profile`과 `profile --json`은 기존 Full Profile v1 계약과 동작을 그대로 유지하고, pet opt-in 결과는 별도의 Full Profile v2 계약으로 구분한다.
...
- 설치 custom pet catalog의 privacy-safe 표시 이름과 호출 시점 숫자 key를 제공하고, CLI 방향키 selector와 module key 선택을 지원한다.
- pet 선택 우선순위를 명시적 key, 명시적 selector, Codex Desktop 선택, 대화형 fallback, unavailable 순서로 고정한다.
...
- 설치돼 있지만 선택 여부가 불확실한 custom pet을 사용자 입력 없이 첫 항목 또는 유일 항목으로 암묵 선택하는 동작
...
- 명시적 catalog key가 없으면 authoritative한 현재 선택 상태와 매칭되는 custom pet을 기본값으로 사용한다. 선택 source를 확정할 수 없을 때 기본 pet 또는 첫 manifest로 추측하지 않으며, TTY selector나 module callback의 명시적 선택만 허용한다.
- Catalog key는 현재 호출의 결정적 정렬 결과에 부여한 1-based 정수로, raw directory ID나 path를 노출하지 않는다. Catalog가 바뀌면 번호가 달라질 수 있으므로 module과 자동화는 목록 조회 후 같은 실행 흐름에서 key를 사용한다.
- `profile --include-pet` human mode는 Desktop 선택이 usable하면 질문 없이 사용하고, usable하지 않으면 TTY에서 방향키 selector를 제공한다. `--select-pet`은 Desktop 선택과 무관하게 selector를 강제한다.
- `profile --json --include-pet`은 자동 입력 대기를 피하기 위해 Desktop 선택 또는 `--pet-key`만 사용하고, `--select-pet`을 함께 명시한 TTY에서만 selector를 허용한다.
- Public root JavaScript SDK는 유지하고 experimental profile module은 별도 package subpath로 노출한다.
...
| `codex-usage-analyzer/experimental-profile` module 사용법 | 공식 실험 API 문서 | Node module consumer | `docs/experimental-full-profile.md` | 별도 API 문서 신규 생성 | Full Profile 계약과 catalog key·선택 우선순위를 한 문서에서 함께 버전 관리한다. |
...
- **Stage 2.5 — pet catalog·선택 계약 보완**
  - Desktop 선택 기본값을 보존하면서 multiple custom pet catalog, 숫자 key와 명시적 module 선택 계약을 구현한다.
  - 암묵적 첫 항목 fallback 금지, catalog privacy, key 오류와 선택 취소를 synthetic fixture로 검증한다.
...
- Stage 2.5
  - `npm test`
  - multiple pet catalog, deterministic 1-based key, explicit override, Desktop selected default와 no-implicit-fallback test
  - catalog item privacy, invalid/stale key와 v2 reason schema/type 정합성 확인
- Stage 3
  - `npm test`
  - `profile`, `profile --json`, `profile --include-pet`, `profile --json --include-pet`, `--pet-key`, `--select-pet`, help/invalid argument CLI test
  - Desktop selected 기본값, human TTY fallback, JSON non-interactive와 forced selector matrix
...
- opt-in v2 출력은 Desktop selected 또는 사용자가 catalog에서 명시적으로 고른 custom pet만 제한된 payload로 표현하고 모든 오류는 안전하게 축소된다.
...
- **선택 상태 source drift**: 과거 `selected-avatar-id`가 현재 앱에서 유지되지 않을 수 있다. Authoritative source를 찾지 못하면 설치 pet으로 암묵 추측하지 않고 명시적 selector/key가 있을 때만 선택하며, 그 밖에는 unavailable 처리한다.
- **Catalog key 변동**: 설치·삭제나 정렬 입력 변경 시 1-based key가 달라질 수 있다. Key를 영구 식별자로 문서화하지 않고 목록 조회와 선택을 같은 흐름에서 수행한다.
- **대화형 자동화 정지**: JSON/non-TTY 호출이 selector 입력을 기다리면 downstream이 멈출 수 있다. JSON은 명시적 `--select-pet`과 TTY가 함께 있을 때만 prompt를 허용한다.
...
- Codex Desktop 선택 custom pet을 기본값으로 사용하되, privacy-safe catalog key·CLI selector·module option으로 다른 custom pet을 명시 선택할 수 있게 하고 built-in image extraction, animation, Tokenmon 코드 변경은 제외하는 범위를 승인받는다.

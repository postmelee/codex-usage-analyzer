...
npm test
node --input-type=module -e 'import {readExperimentalPet} from "./src/experimental-pet.js"; const result=await readExperimentalPet({codexHome:"./src/__tests__/fixtures/experimental-pet/selected-custom"}); if(result.status!=="ok"||result.kind!=="custom"||result.image?.role!=="spritesheet"||typeof result.image?.base64!=="string") process.exit(1); console.log(JSON.stringify({status:result.status,kind:result.kind,role:result.image.role,contentType:result.image.contentType,width:result.image.width,height:result.image.height,byteLength:result.image.byteLength,sha256Length:result.image.sha256.length,base64Length:result.image.base64.length}))'
